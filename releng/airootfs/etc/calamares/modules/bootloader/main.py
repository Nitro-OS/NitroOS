...
def efi_label(efi_directory):
    ...
    if "efiBootloaderId" in libcalamares.job.configuration:
        ...
        if branding and "bootloaderEntryName" in branding:
            efi_bootloader_id = branding["bootloaderEntryName"]
        else:
            efi_bootloader_id = "nitroos"
...
def efi_word_size():
    ...
    try:
        ...
        efi_bitness = sysfile.read(2).strip()
    except Exception:
...
def efi_boot_next():
    ...
    try:
        ...
        libcalamares.utils.warning(f"Failed to set efi_boot_next: {e}")


def get_kernels(installation_root_path):
    """
    Gets a list of kernels and associated values for each kernel.
    """
    kernel_search_path = libcalamares.job.configuration.get("kernelSearchPath", "/usr/lib/modules")
    kernel_pattern = libcalamares.job.configuration.get("kernelPattern", "vmlinuz")

    kernel_list = []

    # find all the installed kernels
    target_search = os.path.join(installation_root_path, kernel_search_path.lstrip('/'))
    if os.path.exists(target_search):
        for root, dirs, files in os.walk(target_search):
            for file in files:
...
def get_grub_efi_parameters():
    ...
    depending on the host machine architecture.
    """
    import platform
...
def run_grub_mkconfig(partitions, output_file):
    ...
    """
    grub_mkconfig_cmd = libcalamares.job.configuration.get("grubMkconfig", "grub-mkconfig")
    installation_root_path = libcalamares.globalstorage.value("rootMountPoint")
    if installation_root_path:
        target_dir = os.path.join(installation_root_path, os.path.dirname(output_file).lstrip("/"))
        os.makedirs(target_dir, exist_ok=True)

    if any([is_zfs_root(partition) for partition in partitions]):
        check_target_env_call(["sh", "-c", "ZPOOL_VDEV_NAME_PATH=1 " +
                               grub_mkconfig_cmd + " -o " + output_file])
    else:
        check_target_env_call([grub_mkconfig_cmd, "-o", output_file])
...
def run_grub_install(fw_type, partitions, efi_directory, install_hybrid_grub):
    ...
    """
    grub_install_cmd = libcalamares.job.configuration.get("grubInstall", "grub-install")
    ...
    if fw_type == "efi":
        ...
        if is_zfs:
            ...
            check_target_env_call([grub_install_cmd,
                                   "--target=i386-pc",
                                   "--recheck",
                                   "--force",
                                   boot_loader_install_path])
...
def install_grub(efi_directory, fw_type, install_hybrid_grub):
    ...
    if fw_type == "efi" or install_hybrid_grub:
        ...
        if not os.path.exists(install_efi_directory_firmware):
            os.makedirs(install_efi_directory_firmware, exist_ok=True)
        ...
        if libcalamares.job.configuration.get(fallback, True):
            libcalamares.utils.debug(f"  .. installing '{efi_boot_file}' fallback firmware")
            efi_file_source = None
            if os.path.exists(install_efi_directory_firmware):
                for root, dirs, files in os.walk(install_efi_directory_firmware):
                    for f in files:
                        if f.lower() == efi_grub_file.lower():
                            efi_file_source = os.path.join(root, f)
                            break
                    if efi_file_source:
                        break

            if efi_file_source and os.path.exists(efi_file_source):
                efi_file_target = os.path.join(install_efi_boot_directory, efi_boot_file)
                try:
                    shutil.copy2(efi_file_source, efi_file_target)
                    libcalamares.utils.debug(f"Copied EFI fallback from {efi_file_source} to {efi_file_target}")
                except Exception as copy_err:
                    libcalamares.utils.warning(f"Could not copy EFI fallback: {copy_err}")
            else:
                libcalamares.utils.warning(f"Could not locate {efi_grub_file} to create EFI fallback")
    ...
    grub_cfg = libcalamares.job.configuration.get("grubCfg", "/boot/grub/grub.cfg")
    run_grub_mkconfig(partitions, grub_cfg)
...
def install_secureboot(efi_directory):
    ...
    grub_probe_cmd = libcalamares.job.configuration.get("grubProbe", "grub-probe")
    efi_boot_mgr_cmd = libcalamares.job.configuration.get("efiBootMgr", "efibootmgr")
    grub_mkconfig_cmd = libcalamares.job.configuration.get("grubMkconfig", "grub-mkconfig")
    ...
    check_target_env_call([grub_mkconfig_cmd,
...
def prepare_bootloader(fw_type, install_hybrid_grub):
    ...
    """
    try:
        gs_name = libcalamares.job.configuration.get("efiBootLoaderVar", None)
        if gs_name and libcalamares.globalstorage.contains(gs_name):
    ...
    if efi_boot_loader == "clr-boot-manager":
        ...
        libcalamares.utils.debug("WARNING: unsupported bootloader combination, fallback to grub")
        install_grub(efi_directory, fw_type, install_hybrid_grub)


def run():
    """
    Main entry point for bootloader module.
    """
    ...
    if fw_type == "efi":
        ...
        esp_found = [p for p in partitions if p.get("mountPoint") == efi_system_partition]
        if not esp_found:
    ...
    try:
        ...
        libcalamares.utils.warning(f"Bootloader execution exception: {e}")
        return (_("Bootloader installation error"),
                _("The bootloader could not be installed: {!s}").format(e))
